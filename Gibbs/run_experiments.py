...
def run_sampler_and_evaluate(x_data, x_aux, y_data, var_names, hyperparams,
    ...
    sampler = SpikeSlabGibbsSampler(
        ...
        use_float32=True,
        trace_max_samples=500  # Keep fewer samples in memory
    )
    results = sampler.run(max_iters, burn_in=burn_in, save_traces=True)

    # Get traces from the results dictionary
    upsilon_trace = results["upsilon"]
    gamma_trace = results["gamma"]
    beta_trace = results["log_beta"]
...
def main():
    ...
    if args.dataset == "emtab":
        # Load EMTAB data from data.py (already imported above)
        ...
        # Apply max_genes and max_samples filtering if specified
        if args.max_genes and args.max_genes < adata.n_vars:
            print(f"Limiting to {args.max_genes} genes for testing (out of {adata.n_vars})")
        ...
        if args.max_samples and args.max_samples < adata.n_obs:
            print(f"Limiting to {args.max_samples} samples for testing (out of {adata.n_obs})")
            labels = adata.obs[label_col]
            from sklearn.model_selection import train_test_split
            _, sample_subset, _, _ = train_test_split(
                np.arange(adata.n_obs), 
                labels,
                test_size=args.max_samples/adata.n_obs,
                stratify=labels,
                random_state=42
            )
            adata = adata[sample_subset]
            print(f"Filtered dataset shape: {adata.shape}")
