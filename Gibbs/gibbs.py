...
class SpikeSlabGibbsSampler:
    """Simplified Gibbs sampler using a spike–and–slab prior for ``upsilon``.

    This class implements the algorithm sketched in the notebook snippet found
    in the user question.  It mirrors the generative process used in
    :class:`GibbsSampler` but uses a different prior for ``upsilon`` along with a
    Metropolis–Hastings step.  The code is intentionally lightweight and does
    not attempt to be as feature rich as ``GibbsSampler`` above.  It provides a
    clear reference implementation that is easy to follow.
    """
