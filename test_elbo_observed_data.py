#!/usr/bin/env python3
"""
Test script to verify that the ELBO is reasonable when using observed data X.
"""
...
def test_elbo_observed_data():
    """Test that ELBO is reasonable when using observed data X."""
    print("=== Testing ELBO with Observed Data X ===")
    ...
    # Compute expected rate for observed data
    expected_rate = jnp.sum(E_theta[:, None, :] * E_beta[None, :, :], axis=2)
    print(f"\nExpected rate shape: {expected_rate.shape}")
    print(f"Expected rate range: {expected_rate.min():.2f} to {expected_rate.max():.2f}")
    ...
    if total_elbo > 0:
        print("❌ WARNING: ELBO is still positive! This might indicate an issue.")
        return False
    elif total_elbo < -1e6:
...
if __name__ == "__main__":
    success = test_elbo_observed_data()
    if success:
        print("\n🎉 ELBO fix with observed data is working!")
    else:
        print("\n💥 ELBO fix with observed data failed!") 